...
import { HOOK_TYPES, HookType, MockReel } from "@/lib/reelRadarData";
...
export default function RadarTab({
  ...
  const [rewritten, setRewritten] = useState<Record<string, "rewrite" | "remix">>({});
  ...
  const handleGenerate = (id: string, mode: "rewrite" | "remix") => {
    setBusyId(id);
    setTimeout(() => {
      setBusyId(null);
      setRewritten((prev) => ({ ...prev, [id]: mode }));
    }, 900);
  };
    ...
    <div>
      <div className="flex items-start justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          >
            {scanning ? "…" : t("reelradar.scanButton")}
          </button>
        </div>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <StatCard label={t("reelradar.statAccounts")} value={16} />
        <StatCard label={t("reelradar.statAnalyzed")} value={40} />
        <StatCard label={t("reelradar.statViews")} value="1.2M" />
        <StatCard label={t("reelradar.statTopHook")} value="先亮结果" />
      </div>
      ...
      <div className="space-y-4">
        {visible.map((reel) => {
          ...
          const genResult = rewritten[reel.id];
          return (
            <div key={reel.id} className="rounded-2xl border border-border bg-surface overflow-hidden">
              <div className="flex flex-col sm:flex-row">
                ...
                <div className="flex-1 p-4 space-y-3 min-w-0">
                  ...
                  {showTranscript && (
                    <p className="text-xs text-foreground/70 leading-relaxed bg-background rounded-lg p-3">
                      {reel.transcript}
                    </p>
                  )}
                  ...
                  {genResult && (
                    <div className="rounded-xl brand-gradient-soft border border-brand-pink/20 p-3 text-xs">
                      <p className="font-medium text-brand-pink mb-1">
                        {genResult === "rewrite" ? t("reelradar.rewriteResultTitle") : t("reelradar.remix")}
                      </p>
                      <p className="whitespace-pre-wrap leading-relaxed text-foreground/90">
                        {genResult === "rewrite" ? reel.rewrite : reel.remix}
                      </p>
                    </div>
                  )}
                </div>
