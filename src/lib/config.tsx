...
const DEFAULT_DISCOVER_KEYWORDS = ["AI 工具", "AI automation", "AI tools for business", "ChatGPT 教学", "vibe coding", "AI agent"];

const DEFAULT_COMPETITOR_ACCOUNTS: CompetitorAccount[] = [
  { id: "acc1", handle: "@smartkids.edu", lastScan: "2026-08-10 09:12", paused: false },
  { id: "acc2", handle: "@studyhub.my", lastScan: "2026-08-10 09:12", paused: false },
  { id: "acc3", handle: "@brightpath.tuition", lastScan: "2026-08-10 09:12", paused: false },
  { id: "acc4", handle: "@examwhiz", lastScan: "2026-08-10 09:12", paused: false },
  { id: "acc5", handle: "@futurekids.center", lastScan: "2026-08-09 22:40", paused: true },
  { id: "acc6", handle: "@genius.tuition", lastScan: "2026-08-10 09:12", paused: false },
];

const DEFAULT_SCAN_SETTINGS: ScanSettings = {
  domainDescription: "马来西亚课外补习教育，服务对象是 UPSR / PT3 / SPM 家长",
  sellingPoint: "小班教学 + AI 学习报告，帮助孩子在 3 个月内看到成绩进步",
  reelsPerAccount: 10,
};
...
interface ConfigValue {
  ...
  addCompetitorAccount: (handle: string) => void;
  removeCompetitorAccount: (id: string) => void;
  toggleCompetitorPause: (id: string) => void;

  useMyTone: boolean;
  setUseMyTone: (v: boolean) => void;

  scanSettings: ScanSettings;
  updateScanSettings: (patch: Partial<ScanSettings>) => void;
...
export function ConfigProvider({ children }: { children: ReactNode }) {
  ...
  const [discoverKeywords, setDiscoverKeywords] = useState(DEFAULT_DISCOVER_KEYWORDS);
  const [competitorAccounts, setCompetitorAccounts] = useState(DEFAULT_COMPETITOR_ACCOUNTS);
  const [useMyTone, setUseMyTone] = useState(true);
  ...
  const value: ConfigValue = {
    ...
    addDiscoverKeyword: (keyword) =>
      setDiscoverKeywords((k) => (keyword.trim() && !k.includes(keyword.trim()) ? [...k, keyword.trim()] : k)),
    removeDiscoverKeyword: (keyword) => setDiscoverKeywords((k) => k.filter((x) => x !== keyword)),

    competitorAccounts,
    addCompetitorAccount: (handle) => {
      const clean = handle.trim().replace(/^@?/, "@");
      if (!handle.trim()) return;
      setCompetitorAccounts((accs) => [
        ...accs,
        { id: `acc_${Date.now()}`, handle: clean, lastScan: "—", paused: false },
      ]);
    },
    removeCompetitorAccount: (id) => setCompetitorAccounts((accs) => accs.filter((a) => a.id !== id)),
    toggleCompetitorPause: (id) =>
      setCompetitorAccounts((accs) => accs.map((a) => (a.id === id ? { ...a, paused: !a.paused } : a))),

    useMyTone,
    setUseMyTone,

    scanSettings,
    updateScanSettings: (patch) => setScanSettings((s) => ({ ...s, ...patch })),
